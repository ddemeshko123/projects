# Advanced solvers for numerical PDE

Course description:

Partial differential equations (PDEs) describe many phenomena around us: from tsunami propagation to neural currents in the human head. Given today’s accuracy demands in industrial and academic applications, numerical solution of the PDEs typically involves millions and even billions of unknowns. This course presents and analyses efficient algorithms for their fast and economical solution. We will concentrate on diffusive, time-harmonic electromagnetic and acoustic systems and models of fluid dynamics. Special emphasis will be on the design of the solvers for anisotropic and/or highly heterogeneous problems, problems on nonuniform and unstructured computational grids. Numerical modeling with both standard (finite-difference) and advanced discretization methods will be considered (finite volume, finite element, mixed finite element). We discuss established approaches based on iterative solvers (Krylov subspace solvers) and robust economical preconditioners (FFT-related, geometric and algebraic multigrid, and others) for the arising large discrete problems. We also look at the possibilities of deep learning (PINNs) and dimensionality reduction methods for numerical solution of PDEs. This course is intended to attract student of all majors interested in mathematical modeling of natural phenomena. Our examples include solid and fluid mechanics, Earth sciences (PDEs arising in geophysical exploration and oil/gas production), and life sciences (established and emerging imaging modalities).

1. HW1 - Simulation of a temperature change of a melon using FEM.
2. HW2 - Simulation of the electrical potential of a ring using FEM.
3. HW3 - Solving of 2D nonlinear viscous Burgers' equation system.
4. HW4 - Simulation of a stationary incompressible sourceless Stokes flow over a step.
